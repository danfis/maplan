...
static void statePoolInit(state_pool_t *pool, const char *fn);
static void statePoolFree(state_pool_t *pool);
static int statePoolNext(state_pool_t *pool, plan_state_t *state);
...
static void statePoolReset(state_pool_t *pool)
{
    pool->cur = 0;
}
