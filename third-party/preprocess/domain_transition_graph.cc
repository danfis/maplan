...
void DomainTransitionGraph::addTransition(int from, int to, const Operator &op,
    ...
    for (size_t i = 0; i < pre_post_vec.size(); i++) {
    ...
    for (size_t i = 0; i < pre_post.effect_conds.size(); i++) {
...
void DomainTransitionGraph::addAxTransition(int from, int to, const Axiom &ax,
    ...
    for (size_t i = 0; i < ax_conds.size(); i++)
        if (true) // [cycles]
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
        ...
        for (size_t j = 0; j < vertices[i].size(); j++) {
            if (!is_dominated[j]) {
                ...
                Condition &cond = trans.condition;
                size_t comp = j + 1; // compare transition no. j to no. comp
                    ...
                    if (other_trans.target != trans.target)
                        ...
                        if (cond.size() == 0) {
                            ...
                            for (size_t k = 0; k < cond.size(); k++) {
                                bool cond_k = false;
                                for (size_t l = 0; l < other_trans.condition.size(); l++) {
...
bool are_DTGs_strongly_connected(const vector<DomainTransitionGraph> &transition_graphs) {
    ...
    for (size_t i = 0; i < transition_graphs.size() - 1; i++)
...
void DomainTransitionGraph::dump() const {
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
        ...
        for (size_t j = 0; j < vertices[i].size(); j++) {
            ...
            for (size_t k = 0; k < trans.condition.size(); k++)
