...
typedef struct _plan_heur_add_t plan_heur_add_t;

#define HEUR(parent) bor_container_of((parent), plan_heur_add_t, heur)
...
static void loadOpFact(plan_heur_add_t *h, const plan_problem_t *p);

plan_heur_t *planHeurAddNew(const plan_problem_t *p, unsigned flags)
{
    plan_heur_add_t *h;

    h = BOR_ALLOC(plan_heur_add_t);
    bzero(h, sizeof(*h));
    h->flags = flags;
...
static void initFacts(plan_heur_add_t *h)
{
    int i;
...
static void initOps(plan_heur_add_t *h)
{
    int i;
...
static void addInitState(plan_heur_add_t *h,
...
static void enqueueOpEffects(plan_heur_add_t *h, op_t *op, plan_pq_t *pq)
{
    fact_t *fact;
    int i;
...
static op_t *nextOp(plan_heur_add_t *h)
{
...
static int getCost(const plan_heur_add_t *h, const plan_op_t *op)
{
    int cost;
...
static void addCondEff(plan_heur_add_t *h, int parent_op_id,
                       const plan_op_t *pop,
